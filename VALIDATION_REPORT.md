...
## 2026-07-23 TASK-023 本地验收（交付候选）

- 实现当前用户可见账本账目与本人工资同步 CSV；账目要求有效成员关系，个人账本要求本人 OWNER，工资查询
  固定当前认证用户，软删除记录不导出且响应不包含数据库 ID。
- Node.js 24.18.0 全仓 `pnpm verify` 与 172 项测试通过：API 13 个文件 50 项、移动端 32 个文件 104 项，
  连同原生网关、共享包、校验包和管理端共 172 项；format、lint、typecheck、Prisma validate、OpenAPI
  81/81、Compose、生产构建、文档、清单和差异检查全部通过。
- 隔离 PostgreSQL/Redis 完整 API/Worker E2E 新增覆盖认证、账本成员隔离、情侣共同账本、工资本人隔离、
  软删除排除、0.01 元精度、公式注入防护、日期校验、响应头和脱敏审计，全部通过。
- Google Chrome Headless/CDP 使用实际 API 和 Blob 下载覆盖 320px、375px、480px、日间/暗色及网络失败；
  `scrollWidth === innerWidth`，可见交互区不小于 44px，无未处理运行时错误，CSV 文件实际下载成功。
- 本任务无 Schema、迁移、Worker 或生产依赖变化；异步导出任务、对象存储、邮件与其他业务数据导出不在范围内。
- `pnpm audit --audit-level moderate` 无已知漏洞；宿主 Redis 6.0.16 只有既有最低版本非阻断提示，Compose/CI
  使用 Redis 7.4。
- 功能提交 `5ac941e546aee86c19c2d207265b68af8ce6ad25` 经 PR #34 合入 `main`，合并提交为

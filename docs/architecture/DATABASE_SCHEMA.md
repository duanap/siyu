...
`id, user_id, name, employer_name, pay_day, default_sync_entry, status, idempotency_key, create_request_hash, created_at, updated_at, deleted_at`

- MVP 每名用户最多一条未删除的 `ACTIVE` 档案；创建幂等键按 `user_id + idempotency_key` 唯一。
- 用户归属、创建幂等键和规范载荷哈希为不可变创建事实。

### salary_profile_items

`id, profile_id, item_type, item_code, item_name, amount_cent, sort_order`

- 档案默认模板随档案级联删除；`profile_id + item_code` 唯一，模板金额允许为 0。

### salary_records

`id, user_id, profile_id, salary_month, gross_cent, deduction_cent, net_cent, payment_status, paid_date, entry_id, idempotency_key, create_request_hash, payment_idempotency_key, payment_request_hash, created_at, updated_at, deleted_at`

- 有效记录唯一：未软删除时 `profile_id + salary_month`（PostgreSQL 部分唯一索引）
- 创建幂等键按 `user_id + idempotency_key` 唯一；用户、档案、工资月份、幂等键和规范载荷哈希不可变。
- 明细收入、扣除合计与三个汇总字段在事务提交时校验；已到账记录的月份、金额与明细不可修改。
- 到账幂等键按 `user_id + payment_idempotency_key` 唯一；已到账记录必须有到账幂等键和请求哈希，到账事实
  不可修改。非空 `entry_id` 必须与本人个人账本中的 `INCOME + SALARY` 来源明细双向一致。
...
`id, ledger_id, creator_user_id, name, target_cent, initial_cent, saved_cent, target_date, status, cover_url, note, idempotency_key, create_request_hash, created_at, updated_at, deleted_at`

- `(creator_user_id, idempotency_key)` 唯一；创建人、账本、幂等键和创建请求哈希为不可变创建事实。
- `(ledger_id, creator_user_id)` 复合外键保证创建人可追溯至账本成员；新写入额外要求有效成员。
- `saved_cent` 必须等于 `initial_cent` 加所有未删除存入的合计，并与
  `ACTIVE/COMPLETED` 状态一致；延迟约束在事务提交前校验。
- 软删除设置 `CANCELLED + deleted_at`，保留存入事实和审计链。

### saving_contributions

`id, goal_id, user_id, amount_cent, business_date, note, idempotency_key, create_request_hash, created_at, updated_at, deleted_at`

- `(user_id, idempotency_key)` 唯一，请求哈希不可变；同键异载荷不能重放。
- 新增和修改必须由目标账本当前有效成员执行；应用层只允许记录本人更新或软删除。
- 目标级 advisory transaction lock 串行化存入增改删与汇总重算，不使用增量读改写。

### notifications

`id, user_id, type, title, content, related_type, related_id, read_at, created_at`

- 周期待确认通知使用 `RECURRING_CONFIRMATION_DUE + RECURRING_RUN + run_id`；接收人为当前仍可管理规则的
  活跃成员。
- 条件唯一索引保证同一接收人、同一周期实例最多一条待确认通知；其他通知类型的重复策略留给对应任务定义。
...
工资到账在 `salary_records` 单独保存 `payment_idempotency_key` 和 `payment_request_hash`。未到账记录的到账日、
来源明细和到账幂等事实均为空；已到账记录必须同时具有到账日和到账幂等事实，可按同步选择保留空 `entry_id`。
非空 `entry_id` 与 `entries` 中 `INCOME + SALARY` 来源明细双向一致，金额等于实发工资、业务日期等于到账日，
创建人及个人账本所有者等于工资所属用户。用户级到账幂等键和 `source_type + source_id` 均唯一；双向关系使用
延迟约束触发器校验，已到账事实和 `SALARY` 来源明细的财务/来源字段不可修改，来源明细不得硬删除。
...
- 周期 Worker 通知幂等增量迁移：
  `apps/api/prisma/migrations/20260722060000_recurring_worker_notifications/migration.sql`
- 工资档案、模板与月度记录增量迁移：
  `apps/api/prisma/migrations/20260722100000_salary_records/migration.sql`
- 工资到账与收入联动增量迁移：
  `apps/api/prisma/migrations/20260722120000_salary_mark_paid/migration.sql`
- 攒钱目标与存入增量迁移：
  `apps/api/prisma/migrations/20260722140000_saving_goals/migration.sql`
- 用户默认时区暂按已批准 ExecPlan 使用 `Asia/Shanghai`；若负责人否决，后续迁移移除数据库默认值
- Prisma 无法直接表达的部分唯一索引、复合归属外键与 CHECK 由人工审查的 migration SQL 固化
- 借贷处理、周期实例和工资记录引用的账目禁止硬删除；必须按对应业务规则处理来源关系
...
`saving_goals` 使用 `creator_user_id + idempotency_key`。

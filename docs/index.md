# 项目文档索引

## 开始任务前必读

1. `project-memory/CURRENT_STATE.md`
2. `project-memory/DECISIONS.md`
3. `project-memory/KNOWN_ISSUES.md`
4. 当前任务文件
5. 当前任务相关的产品、设计、架构和质量文档

## 产品

- `product/BRAND_IDENTITY.md`
- `product/PRODUCT_REQUIREMENTS.md`
- `product/FEATURE_SCOPE.md`
- `product/BUSINESS_RULES.md`
- `product/ACCEPTANCE_CRITERIA.md`
- `product/GLOSSARY.md`

## 设计

- `design/INFORMATION_ARCHITECTURE.md`
- `design/SCREEN_FLOW.md`
- `design/UI_DESIGN_GUIDE.md`
- `design/COMPONENT_INVENTORY.md`
- `design/SCREEN_SPECIFICATIONS.md`
- `design/DESIGN_DELIVERY_PLAN.md`
- `design/screens/index.html`

## 架构

- `architecture/TECH_ARCHITECTURE.md`
- `architecture/DATABASE_SCHEMA.md`
- `architecture/schema.prisma`
- `architecture/API_CONTRACT.md`
- `architecture/openapi.yaml`
- `architecture/PERMISSION_MODEL.md`
- `architecture/SECURITY.md`
- `architecture/BACKGROUND_JOBS.md`
- `architecture/DEPLOYMENT.md`

## 项目记忆

- `project-memory/CURRENT_STATE.md`
- `project-memory/DECISIONS.md`
- `project-memory/CHANGELOG_AI.md`
- `project-memory/KNOWN_ISSUES.md`
- `project-memory/TECH_DEBT.md`
- `project-memory/RELEASE_HISTORY.md`

## 计划与质量

- `plans/ROADMAP.md`
- `plans/PLANS.md`
- `plans/TASK_BACKLOG.md`
- `plans/active/`
- `plans/templates/TASK_TEMPLATE.md`
- `quality/TEST_STRATEGY.md`
- `quality/QA_CHECKLIST.md`
- `quality/DEFINITION_OF_DONE.md`
- `quality/RELEASE_CHECKLIST.md`

## Codex

- `codex/CODEX_FIRST_PROMPT.md`
- `codex/TASK_PROMPT_TEMPLATE.md`
- `codex/BUGFIX_PROMPT_TEMPLATE.md`
- `codex/REVIEW_PROMPT.md`
- `codex/MODEL_ASSIGNMENT.md`

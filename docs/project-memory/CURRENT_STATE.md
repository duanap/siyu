...
更新时间：2026-08-04
文档版本：v1.38.0
代码状态：TASK-028 Android APP、品牌图标与骨架屏已完成本地开发和构建，待远程 CI 与真机验收

## 当前阶段

M3 至 M8 的 TASK-000 至 TASK-024 已正式关闭。TASK-025 已完成仓库开发与 Git/CI 闭环，其面向公众发布
验收继续等待外部条件。TASK-026 已按“先满足本人和女朋友使用”目标完成开发、验证、PR、远程 CI、合并
和最终 `main` CI，个人/情侣私有部署的仓库开发正式关闭。

TASK-027 已按负责人选择的“方案 1 / 月度叙事”完成首页与“我的”开发：首页以月结为视觉主角，复用
真实周期实例和本人借贷形成失败隔离的待办；“我的”按共同生活、财务计划、数据与安全分组，并提供金额
隐藏和日间/夜间/跟随系统。功能经 PR #42 合入 `main`，PR CI Run `30839314625` 与 main push CI Run
`30839548270` 全绿，任务正式关闭。

TASK-028 已按 ADR-036 实现固定加载 `https://siyu.duanap.cn` 的 Android WebView 壳，应用名称“四时有余”、
调试包名 `cn.duanap.siyu`。壳只申请联网权限，关闭明文、本地文件/内容、混合内容、第三方 Cookie 和任意
原生桥；站外 Web 链接交给系统浏览器，未知 Scheme 拒绝。Image 2 品牌徽记已进入 Android/PWA 图标和
本地启动骨架，网页 JavaScript 启动前及首页、明细、统计加载态使用统一骨架，底部导航与分类图标映射已集中。
本地 Android lint、单元测试和 debug APK 真构建通过；Node.js 24.14.0 下 lint、typecheck、203 项既有测试
加新增 1 项移动端测试、生产构建、文档检查通过。当前没有连接 Android 设备，真机安装、系统 WebView 行为和
远程 CI 尚未验收，不能标记为正式关闭或应用商店发布完成。
...
- OpenAPI 3.1 覆盖 API_CONTRACT 已批准的 82 个操作，并生成共享类型。
...
- TASK-024 功能提交 `72b38ea8709a2ddc84942df9f4fc734233a88033` 经 PR #36 合入 `main`，合并提交为
  `c8154019544f11ccb30f92c5ecf73744042ac103`；PR CI Run `30209669521` 与 main push CI Run
  `30209769716` 的 `quality`、`database`、`secret-scan` 全部通过，TASK-024 正式关闭。
- TASK-025 已新增失败关闭的生产配置预检、仓库外 PostgreSQL 备份、隔离恢复验证、构建产物 HTTP 冒烟和
  Chrome 三尺寸双主题发布矩阵；CI 数据库任务同步加入真实备份恢复演练。
- 发布预检会拒绝占位 JWT、非 HTTPS、非 Secure Cookie及启用但未配置的外部认证能力；Worker 生产启动
  不接受测试或任意未实现邮件提供方。检查结果不输出秘密。
- 仓库内 187 项测试、12 迁移/约束、完整 API/Worker E2E、PostgreSQL 17 备份恢复、生产构建产物冒烟和
  Chrome 6 组矩阵通过；QQ、邮件、公开域名/运营设施和品牌权属仅继续阻断面向公众发布。
- TASK-025 经 PR #38 Squash merge 合入 `main`，合并提交为
  `7fe0b8b91b40996f9325aa61a02c23f93ef54958`；最终 PR CI Run `30213362056` 与 main push CI Run
  `30213469686` 的 `quality`、`database`、`secret-scan` 全部通过。仓库开发任务已闭环，但不等于生产上线。
- TASK-026 新增 `personal/public` 部署档案、新账号注册/QQ OAuth/邮件密码重置显式能力开关和公开
  `/api/v1/auth/capabilities`；两人受控建号后可关闭注册，禁用能力的服务端端点不产生用户、账本、令牌、
  队列或 Worker 副作用。
- 个人档案在注册、QQ 与邮件重置按需关闭时可通过发布预检；邮箱登录、会话、已登录修改密码及全部财务功能
  保持可用。强 JWT、HTTPS/Secure Cookie、精确 CORS、权限隔离、备份恢复和回滚仍是硬门槛。
- TASK-026 功能提交 `344e4e7d5be0942ccf38759b09255d4e28073a3b` 经 PR #40 合入 `main`，合并
  提交为 `fa36e9567906b087ac494f891be0e2fc63f39601`；PR CI Run `30236916112` 与 main push
  CI Run `30237054972` 的 `quality`、`database`、`secret-scan` 全部通过，TASK-026 正式关闭。
- TASK-027 首页与“我的”使用真实 Ant Design 图标和连续五项底部导航；首页新增月结透明插画、金额隐藏、
  真实周期/本人借贷待办与交易优先最近明细，“我的”新增账号摘要、通知角标、主题和本机隐私设置。
- TASK-027 不修改数据库、迁移或公开 API；情侣首页不读取本人借贷，工资、社保、医保、公积金和借贷的
  本人可见边界保持不变。
...
- TASK-024 PR CI Run `30209669521` 与 main push CI Run `30209769716` 全绿；两轮均覆盖冻结安装、
  Prisma 生成/迁移、完整 PostgreSQL/Redis E2E、生产构建、依赖审计和秘密扫描。
- TASK-025 Node.js 24.18.0 标准 `pnpm verify` 完整通过；最终加固后按同一门禁拆分复核，187 项测试、
  完整 API/Worker E2E、12 个迁移和数据库约束全部通过。
- TASK-025 PostgreSQL 17 custom format 备份经 SHA-256 校验后恢复到 `siyu_restore_task025`，确认 12 个
  有效迁移和 27 张业务表并清理隔离恢复库。
- TASK-025 生产构建的 API 与原生网关真实启动；只读冒烟覆盖健康、手机端、管理端、401 保护和安全响应头。
  Google Chrome Headless/CDP 覆盖登录入口 320px、375px、480px × 日间/暗色 6 组，均无横向溢出或
  小于 44px 的可见交互区。
- TASK-026 Node.js 24.18.0 标准 `pnpm verify` 通过：全仓 197 项测试（脚本 14、移动端 108、API 60、
  其余 15）、文档、清单、format、lint、typecheck、Prisma、OpenAPI 82/82、Compose 和生产构建通过。
- TASK-026 另行完成 12 个迁移的空库/历史升级/重复部署/零差异/26 模型 introspection/数据库约束，
  以及隔离 PostgreSQL/Redis 的认证至后台完整 API/Worker E2E；依赖审计无已知漏洞。
- 个人档案 CLI 发布预检所有项目 PASS；真实 Chrome 登录页 320/375/480px × 日间/暗色 6 组无横向溢出、
  小于 44px 交互区或不可用入口，另目视通过 320px 首页邮箱单入口。
- TASK-027 Node.js 24.18.0 下 lint、typecheck、全仓 203 项测试和生产构建通过；其中移动端 35 个文件
  114 项，API 16 个文件 60 项。隔离 PostgreSQL 17 应用 12 个迁移后，认证、情侣、分类、账目、借贷、
  周期、工资、导出、攒钱、通知和后台完整 API/Worker E2E 通过。
- TASK-027 Chrome Headless/CDP 使用受控本地 API 响应覆盖首页与“我的”320/375/426/480px 日间及
  375px 夜间共 10 组：无横向溢出、无小于 44px 的交互区、五项导航激活正确；金额隐藏点击与本机持久化
  通过，控制台和受控请求无错误。`design-qa.md` 三轮修复后为 `passed`，证据已进入
  `docs/quality/evidence/task027/`；这些结果不代表公网生产验证。
- TASK-027 功能提交 `d9d8a99` 与依赖安全修复 `881b0a3` 经 PR #42 Squash merge 合入 `main`，合并提交
  `27f56dd61791fdf561cc0b00985b49de3d824907`；PR CI Run `30839314625` 与 main push CI Run
  `30839548270` 的 `quality`、`database`、`secret-scan` 全绿。
...
以负责人受控环境建立个人档案配置、备份与回滚点，临时开放注册创建两个账号后关闭注册，并完成一次真实
备份恢复与上线后观察。
QQ、邮件、商标/软著、应用商店、广告及完整公开运营设施留到未来确实面向公众时处理。

## 待负责人确认

- Logo、商标、域名、软著、应用商店名称和安卓包名可用性（仅未来公开/分发前）。
- 默认时区是否保留 `Asia/Shanghai`。
- 周期恢复/跳过/确认金额、短月取月末、来源借贷删除、工资固定支出口径和目标归属已确认。
- 上传、异步导出、反馈、公告与后台扩展数据模型的最小契约；有界同步 CSV 与最小后台已冻结。
- QQ 互联申请已通过；个人档案当前关闭 QQ 与邮件重置，未来启用时再配置 App ID/App Key/严格回调和邮件提供方。

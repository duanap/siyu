...
更新时间：2026-07-14
文档版本：v1.6.0
代码状态：TASK-008 记账页面与明细列表已在任务分支完成实现与本地验收，待 PR

## 当前阶段

M3：身份认证、账本关系、账本级分类、普通账目 API 和移动端记账/明细客户端已建立；统计尚未开始。
...
- 移动端 `/entries`、`/entries/new`、`/entries/:id`，支持账本/月份/类型/分类/创建人/关键词与分页 URL 恢复。
- 快速记账使用字符串/BigInt 金额校验、稳定幂等键和用户时区日期；详情支持能力驱动编辑、冲突与软删除。
- 集中式 Entry 客户端直接复用生成 `components` 类型；401 单飞 Refresh 最多重放一次并尊重 AbortSignal。
- 新增 14 个移动端公共组件、明细底部导航和账号入口；来源账目只读，OWNER/MEMBER 权限不在前端推导。

## 明确未实现

首页与统计、借贷、周期、工资、攒钱、通知、导出和后台业务管理均未实现；
...
- 单元/API/组件测试：全仓 71 项通过（移动端 17 个文件、52 项）；隔离 PostgreSQL 认证、情侣账本、分类和 Entry E2E 全流程通过。
...
- Google Chrome 150.0.7871.114 真浏览器覆盖 `/entries`、`/entries/new`、`/entries/:id`：18 组基础矩阵和
  12 组 Drawer/Dialog 补充矩阵全部通过；无横向溢出，日夜主题、44px、焦点恢复和关键业务状态通过。
- 依赖审计：无已知漏洞。
...
TASK-008 任务分支实现与本地验收已完成，下一步仅为 TASK-008 PR 与远程 CI；TASK-009 尚未开始。

...
更新时间：2026-07-23
文档版本：v1.30.0
代码状态：TASK-022 最小管理后台已合并、CI 通过并正式关闭；下一项为 TASK-023 CSV 导出

## 当前阶段

M3、M4、M5、M6、M7 已完成并合入 `main`；M8 的 TASK-022 已正式关闭，下一项为 TASK-023。
...
除周期待确认外的通知生产链路、通知偏好、邮件/短信/Push、导出、公告、反馈和后台敏感详情均未实现。
...
进入 TASK-023 CSV 导出，先冻结本人账目与工资导出的字段、日期范围、CSV 注入防护和同步响应上限。
...
- 上传、异步导出、反馈、公告与后台扩展数据模型的最小契约；最小状态/关系/任务/审计后台已冻结。

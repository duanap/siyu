...
更新时间：2026-07-27
文档版本：v1.35.0
代码状态：TASK-025 仓库内开发与 Git/CI 已闭环；正式环境与凭据验收阻断，尚未允许生产上线

## 当前阶段

M3、M4、M5、M6、M7 已完成并合入 `main`；M8 的 TASK-022 至 TASK-024 已正式关闭。最后一项 TASK-025
已完成仓库内开发、PR、远程 CI、合并与 main CI 闭环，生产发布验收继续等待外部条件。
...
- 发布预检会在占位 JWT/QQ、非 HTTPS、非 Secure Cookie或缺少已批准生产邮件适配器时拒绝；Worker 生产
  启动不再接受空值或任意未实现邮件提供方。检查结果不输出秘密。
- 仓库内 187 项测试、12 迁移/约束、完整 API/Worker E2E、PostgreSQL 17 备份恢复、生产构建产物冒烟和
  Chrome 6 组矩阵通过；正式生产上线仍被 QQ、邮件、域名/TLS、环境/监控、回滚制品和品牌权属阻断。
...
先完成 TASK-025 代码 PR、远程 CI 与 `main` CI；随后由负责人提供正式邮件供应商选择、QQ 凭据、域名/TLS、
staging/production 访问、监控/回滚制品和品牌权属确认，才能继续正式环境验收并允许上线。

...
更新时间：2026-07-22
文档版本：v1.13.0
代码状态：TASK-008 至 TASK-011 已正式关闭；TASK-012 周期规则和实例已完成本地交付候选，待提交、PR 与远程 CI

## 当前阶段

M3、M4 已完成并合入 `main`；M5 的 TASK-012 周期规则和实例已完成本地交付候选，尚未正式关闭。
...
周期 Worker 调度与手机页面、工资、攒钱、通知、导出和后台业务管理均未实现。
...
提交并推送 `task/TASK-012-recurring-rules`，创建 PR，等待 PR CI 与合并后 main CI 全部通过；随后正式关闭
TASK-012 并进入 TASK-013 BullMQ Worker 与幂等。

## 待负责人确认

- Logo、商标、域名、软著、应用商店名称和安卓包名可用性。
- 默认时区是否保留 `Asia/Shanghai`。
- 工资固定支出口径；周期恢复/跳过/确认金额、短月取月末、来源借贷删除和目标归属已确认。

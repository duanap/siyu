...
更新时间：2026-07-22
文档版本：v1.13.1
代码状态：TASK-008 至 TASK-012 已合并、远程 CI 全部通过并正式关闭；下一项为 TASK-013 BullMQ Worker 与幂等

## 当前阶段

M3、M4 已完成并合入 `main`；M5 的 TASK-012 周期规则和实例已正式关闭，准备进入 TASK-013。
...
创建 TASK-013 ExecPlan，实现 BullMQ 周期扫描、稳定 Job ID、退避重试、Worker 并发安全与运行可观测性，
复用 TASK-012 的到期物化领域入口，不复制财务写入逻辑。

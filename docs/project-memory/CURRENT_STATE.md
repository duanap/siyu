...
更新时间：2026-07-22
文档版本：v1.21.0
代码状态：TASK-018 工资页面已完成本地交付候选与全部质量门；待提交、PR、远程 CI 和正式关闭

## 当前阶段

M3、M4、M5 已完成并合入 `main`；M6 的 TASK-015、TASK-016、TASK-017 已正式关闭，TASK-018 已完成
本地交付候选，等待远程闭环。
...
通知读取/已读 API、攒钱、导出、人工任务重试和后台业务管理均未实现。
...
提交 TASK-018 本地交付候选，创建 PR，等待远程 `quality`、`database`、`secret-scan`，合并后同步 main CI
与项目记忆并正式关闭，然后进入 TASK-019 攒钱目标 API。

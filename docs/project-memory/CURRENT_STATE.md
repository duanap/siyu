...
更新时间：2026-07-22
文档版本：v1.16.1
代码状态：TASK-015 已合并、远程 CI 全部通过并正式关闭；下一项为 TASK-016 工资到账与收入联动

## 当前阶段

M3、M4、M5 已完成并合入 `main`；M6 的 TASK-015 已正式关闭，准备进入 TASK-016。
...
通知读取/已读 API、工资到账/余额/年度统计/页面、攒钱、导出、人工任务重试和后台业务管理均未实现。
...
创建 TASK-016 ExecPlan，实现工资到账状态迁移、幂等确认、本人普通收入 Entry 联动、来源双向一致性和审计。

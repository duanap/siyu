...
## 2026-07-26 / TASK-024 正式关闭

### 关闭证据

- 功能提交 `72b38ea8709a2ddc84942df9f4fc734233a88033` 已推送并经 PR #36 Squash merge 合入
  `main`；合并提交为 `c8154019544f11ccb30f92c5ecf73744042ac103`。
- PR CI Run `30209669521` 与 main push CI Run `30209769716` 的 `quality`、`database`、
  `secret-scan` 全部通过；两轮均覆盖冻结安装、迁移、完整 PostgreSQL/Redis E2E、构建、依赖审计和秘密扫描。
- 项目状态、任务清单、已知问题和 ExecPlan 已同步为正式关闭状态；正式环境与凭据验收仍按既有边界进入
  TASK-025，不将仓库 CI 冒充为生产上线验收。
...
## 2026-07-27 / TASK-025 仓库开发闭环

### 闭环证据

- 发布验收功能经 PR #38 Squash merge 合入 `main`，合并提交为
  `7fe0b8b91b40996f9325aa61a02c23f93ef54958`。
- 最终 PR CI Run `30213362056` 与 main push CI Run `30213469686` 的 `quality`、`database`、
  `secret-scan` 全部通过；两轮均覆盖完整测试、迁移、E2E、构建、依赖审计和秘密扫描。
- 新增数据库门禁在 PR 与 main 两轮 CI 中均完成 PostgreSQL 真实备份、`siyu_restore_*` 隔离恢复、
  12 个有效迁移、27 张业务表、约束验证和恢复库清理。
- PR CI 首轮暴露 `pnpm` 参数分隔符兼容问题，次轮暴露测试默认参数读取全局邮件环境的问题；均在任务分支
  根因修复并加入确定性回归后通过，未跳过、重试隐藏或弱化门禁。

### 当前状态

TASK-025 的仓库内开发、验证、文档、提交、PR、远程 CI、合并和最终 `main` CI 已闭环。正式生产上线验收
仍被 REL-025-01 至 REL-025-06 阻断，当前决定保持“不允许生产上线”；不得把仓库闭环标记为真实 QQ、
邮件、域名、基础设施、监控回滚或品牌法务验收完成。

## 2026-07-27 / TASK-026 个人/情侣私有部署

### 修改内容

- 按负责人“先满足本人和女朋友使用”的目标新增 `personal/public` 部署档案；公开商业发布条件不再阻断
  个人档案，仍保留全部财务安全门槛。
- 新增新账号注册、QQ OAuth、邮件密码重置三项服务端能力开关和公开认证能力接口；禁用入口不显示，
  直接请求返回 503，且不创建用户/账本、重置令牌、队列任务或密码重置 Worker。
- 个人档案支持受控建号：临时开放注册创建两个账号后关闭；邮箱登录、会话和已登录修改密码保持可用。
- 发布预检、原生配置、Compose、环境示例、OpenAPI 82/82、登录/首页、部署/安全/设计/质量文档已同步。

### 数据库与 API 变化

- 无 Prisma Schema 或迁移变化。
- 新增 `GET /api/v1/auth/capabilities`，返回 `emailPassword/registration/qqOAuth/passwordReset`。
- 注册、QQ 和密码重置现按显式能力失败关闭；既有邮箱登录和财务 API 契约不变。

### 当前验证

- Node.js 24.18.0 标准 `pnpm verify` 通过；全仓 197 项测试：脚本 14、移动端 108、API 60、其余 15。
- 12 个迁移的空库/历史升级/重复部署/零差异/26 模型 introspection/数据库约束通过；隔离
  PostgreSQL/Redis 完整 API/Worker E2E 覆盖所有既有业务模块并通过。
- OpenAPI 82/82、Compose、原生配置、生产构建、个人档案 CLI 发布预检和依赖审计通过。
- Google Chrome Headless/CDP 登录页 320/375/480px × 日间/暗色 6 组无横向溢出或小于 44px 交互区；
  320px 首页邮箱单入口截图目视通过。证据仅在 `/tmp`，未进入仓库。

### 当前状态

功能提交 `344e4e7d5be0942ccf38759b09255d4e28073a3b` 经 PR #40 Squash merge 合入 `main`，合并
提交为 `fa36e9567906b087ac494f891be0e2fc63f39601`；PR CI Run `30236916112` 与 main push CI Run
`30237054972` 的 `quality`、`database`、`secret-scan` 全部通过，TASK-026 正式关闭。尚未对负责人
真实环境执行部署、两个账号建号、备份恢复或上线后观察，不将仓库闭环冒充已部署。

## 2026-08-03 / TASK-027 首页与“我的”APP 化重构

### 修改内容

- 按负责人选定的“方案 1 / 月度叙事”重构首页：月结主卡、金额隐藏、真实周期/本人借贷待办、交易优先最近
  明细和个人/情侣差异化文案；情侣账本不会请求本人借贷。
- 重构“我的”为账号摘要、共同生活、财务计划、数据与安全、外观与隐私分组；接入真实通知未读数、三档
  主题、本机金额隐私和防重复退出。
- 五项底部导航和最近明细改用 Ant Design 图标，中央“记一笔”保持突出；新增透明月结财务插画和底部安全区。
- 新增 TASK-027 浏览器 QA 脚本、设计对照报告与持久化截图证据，修复 44px 点击区、图标/内容层级和分组
  卡片三类 P2 差异。

### 数据库与 API 变化

- 无 Prisma Schema、迁移或公开 API 变化。
- 只读取既有账本、统计、账目、周期实例、本人借贷和本人通知；主题与金额隐藏只保存在本机。

### 验证

- Node.js 24.18.0 下 lint、typecheck、全仓 203 项测试和生产构建通过；移动端为 35 个文件 114 项。
- 隔离 PostgreSQL 17 应用 12 个迁移，隔离 Redis 7.4 下完整 API/Worker E2E 通过。
- Chrome Headless/CDP 覆盖首页与“我的”10 组移动端日夜矩阵，无横向溢出、小点击区、控制台或受控请求错误；
  金额隐藏点击及持久化通过。
- 归一化视觉对照完成三轮修复，`design-qa.md` 最终结果为 `passed`。

### 当前状态

TASK-027 功能提交 `d9d8a99` 经 PR #42 进入远程 CI。首次 Run `30838781197` 的依赖审计发现
`brace-expansion` 与 PostCSS 新公告后保持失败，提交 `881b0a3` 更新安全覆盖；PR CI Run

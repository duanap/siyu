...
| 05A | 账目详情 | `/entries/:id` | 完整字段、服务端能力、编辑、删除和来源只读 | 加载、错误、无权限、冲突、提交中、夜间 |
...
账目页面使用服务端 `canEdit/canDelete`，普通客户端不得维护非 `MANUAL` 来源账目。明细筛选和分页深度
写入 URL；快速记账的幂等键在失败重试期间保持不变；编辑和删除使用 `expectedVersion`。
...
- 页面组件和路由
- API 状态与错误映射
- 完成并记录 320px、375px、480px 的日间和夜间真浏览器验证矩阵
- 临时截图仅可作为仓库外验收辅助，浏览器用户目录和缓存不得进入 Git，验收结束后必须清理
- 空状态和错误状态
- 关键交互测试
- 对应验收条件编号
